...
if len(pair_A) == 2:
    ...
    if d1['[B] (M)'] != d2['[B] (M)']:
        ...
        try:
            if any(val in [0, None, '', np.nan] for val in [v1, v2, A1, A2]):
                st.error("Nilai [A] dan Laju (v) tidak boleh nol, kosong, atau NaN.")
                st.stop()
            ...
            with st.expander("🔍 Debug: Data untuk orde A"):
...
if len(pair_B) == 2:
    ...
    if d1['[A] (M)'] != d2['[A] (M)']:
        ...
        with st.expander("🔍 Debug: Data untuk orde B"):
            st.write(f"v1 = {v1}, v2 = {v2}")
            st.write(f"B1 = {B1}, B2 = {B2}")

        try:
            if any(val in [0, None, '', np.nan] for val in [v1, v2, B1, B2]):
                st.error("Nilai [B] dan Laju (v) tidak boleh nol, kosong, atau NaN.")
                st.stop()

            ratio_v = Fraction(v2, v1)
            ratio_B = Fraction(B2, B1)

            with st.expander("📊 Rasio yang dihitung"):
                st.write(f"ratio_v = {float(ratio_v):.4f} (v2/v1)")
                st.write(f"ratio_B = {float(ratio_B):.4f} (B2/B1)")
...
if x is not None and y is not None:
    ...
    try:
        ...
        st.warning(f"Gagal menghitung konstanta laju k: {e}")
